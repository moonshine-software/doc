...
- `table_row_updated:{componentName}`(`JsEvent::TABLE_ROW_UPDATED`) - row update in the table,
- `table_row_added:{componentName}`(`JsEvent::TABLE_ROW_ADDED`) - append new cloned row,
- `table_empty_row_added:{componentName}`(`JsEvent::TABLE_EMPTY_ROW_ADDED`) - append/prepend new row,
...
Open Modal with `asyncUrl` to load new asynchronous content.
