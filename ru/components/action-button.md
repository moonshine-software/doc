...
## Уникальные имена для модальных окон и OffCanvas

В последнем обновлении компонент `ActionButton` был улучшен для автоматической генерации уникальных имен для модальных окон и компонентов OffCanvas. Это достигается путем добавления случайной строки к идентификатору компонента, что гарантирует уникальность каждого экземпляра.

### Использование

При использовании методов `inModal()` или `inOffCanvas()` больше не нужно вручную указывать уникальное имя. Система автоматически сгенерирует уникальный идентификатор, используя случайную строку в сочетании с ключом данных компонента.


Это изменение помогает предотвратить конфликты, когда на одной странице используется несколько модальных окон или компонентов OffCanvas.

> [!TIP]
> Для получения дополнительной информации о использовании модальных окон и компонентов OffCanvas, обратитесь к разделам [Modal](/docs/{{version}}/components/modal) и [OffCanvas](/docs/{{version}}/components/off-canvas).
