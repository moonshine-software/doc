...
По умолчанию активна `MoonShine\ColorManager\Palettes\PurplePalette`.
...
Далее мы последовательно разберём базовые методы, способы получения значений и готовые хелперы для компонентов.
...
Примеры ниже показывают три частых сценария: базовые настройки фона и текста, интерактивные элементы навигации и формы со всеми состояниями.
...
Комбинируйте описанные подходы: начните с выбора палитры, при необходимости настройте отдельные переменные через `set()` или `setEverything()`,
затем примените шорткаты для пакетной правки компонентов и, наконец, выведите итоговые значения через `toHtml()` для быстрой проверки в интерфейсе.
