...
Методы `metrics()`, `queryTags()`, `filters()` перенесены в класс индексной страницы.

Метод `indexButtons()` удален из ресурса, вместо него следует использовать метод `buttons()` в соответствующей индексной странице.

Метод `topButtons()` удален из ресурса, вместо него следует использовать методы `topLeftButtons()` и `topRightButtons()` в соответствующей индексной странице.

Метод `formButtons()` удален из ресурса, вместо него следует использовать метод `buttons()` в соответствующей странице формы.

Метод `formBuilderButtons()` удален из ресурса, вместо него следует использовать метод `formButtons()` в соответствующей странице формы.

Свойства `$clickAction`, `$stickyTable`, `$stickyButtons`, `$columnSelection` удалены из ресурса.
Вместо них следует переопределять метод `modifyListComponent()`.

Это не полный список изменений в ресурсе, но процесс переноса всего соответствующего функционала
из ресурса в CRUD-страницы довольно интуитивный и не должен составить труда.
