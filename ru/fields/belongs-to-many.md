...
## Режим модального окна для Pivot

Поле `BelongsToMany` теперь поддерживает режим модального окна для управления записями в таблице связей через интерфейс модального окна. Эта функция обеспечивает более удобный способ добавления, редактирования и удаления связанных записей в связи `BelongsToMany`.

### Включение режима модального окна

Чтобы включить режим модального окна, используйте метод `pivotModalMode()` для вашего поля `BelongsToMany`:


### Возможности

- **Создание и редактирование**: Добавление или обновление записей через модальную форму.
- **Удаление**: Удаление записей с подтверждением.
- **Настройка**: Изменение кнопок создания, редактирования и удаления с помощью замыканий.

### Пример


> [!NOTE]
> Режим модального окна для pivot несовместим с функциями асинхронного поиска или реактивности.
