...
> [!NOTE]
> Параметр `$column` работает на основе атрибута `name` поля в DOM.
> Если форма добавляет префиксы к полям (например, в фильтрах), то `name` будет иметь вид `filter[column]`,
> и в `showWhen` нужно указывать `'filter.column'`.
> Для итерируемых элементов с индексами (например, `name="attributes[0][column]"`)
> параметр `$column` должен соответствовать структуре: `'attributes.0.column'`.
