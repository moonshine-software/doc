...
## Интеграция с PrettyLimit

Поле `Text` теперь поддерживает функцию `PrettyLimit`, позволяющую отображать текст с ограничением по количеству символов в визуально привлекательной форме.

### Включение PrettyLimit

Чтобы включить `PrettyLimit` для поля `Text`, используйте метод `prettyLimit()`:


Это применит компонент `PrettyLimit` к текстовому полю, усечая текст, если он превышает заданный лимит, и позволяя его расширение при наведении.
